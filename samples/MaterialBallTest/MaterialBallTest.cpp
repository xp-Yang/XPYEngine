...
void MaterialBallTest::spawnGallery(Scene *scene)
{
    ...
    ModelImporter shader_ball_importer;
    ...
    auto createBallObject = [scene, this, &shader_ball_sub_meshes](const std::string &name, const std::shared_ptr<Material> &material) -> GObject *
    {
        GObject *ball = scene->createObject(name);
        if (!ball)
            return nullptr;

        MeshComponent &mesh = ball->addComponent<MeshComponent>();
        mesh.source_filepath = m_mesh_path;
        for (const auto &template_sub_mesh : shader_ball_sub_meshes)
        {
            if (!template_sub_mesh || !template_sub_mesh->geometry)
                continue;

            auto sub_mesh = std::make_shared<Mesh>(template_sub_mesh->geometry, material);
            sub_mesh->sub_mesh_idx = template_sub_mesh->sub_mesh_idx;
            sub_mesh->index_offset = template_sub_mesh->index_offset;
            sub_mesh->index_count = template_sub_mesh->index_count;
            sub_mesh->translation = template_sub_mesh->translation;
            sub_mesh->rotation = template_sub_mesh->rotation;
            sub_mesh->scale = template_sub_mesh->scale;
    ...
    for (size_t r = 0; r < layout.row_counts.size() && mat_idx < layout.materials.size(); ++r)
    {
        ...
        for (int c = 0; c < count && mat_idx < layout.materials.size(); ++c)
        {
            ...
            GObject *ball = createBallObject("Ball_" + mat_name, it->second);
            if (!ball)
                continue;
