...
{
    auto texture = std::make_shared<Texture>(TextureType::Custom, std::string(ASSET_DIR) + "/images/pure_white_map.png", false);
...
void GcodePreview::rebuildMeshRange(const std::array<LinesBatch, ExtrusionRole::erCount> &lines_batches)
{
    ...
    for (int i = 0; i < static_cast<int>(sub_meshes.size()); i++)
    {
        ...
        const auto &index_offset = is_colored ? m_gcode_trace->colored_indices_interval(ExtrusionRole(role_idx)) : m_gcode_trace->colorless_indices_interval(ExtrusionRole(role_idx));
