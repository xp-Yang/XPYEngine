...
| GPU 资源创建 | `RenderMeshResource`、`RenderMaterialResource`、`RenderTextureData` |
...
flowchart LR
    ...
    CPU --> RTD["RenderTextureData"]
    RTD --> GPU["RhiTexture"]
```

- CPU `Texture` 持有像素数据；`RenderTextureData` 构造时调用 `Rhi::newTexture()` 上传。
...
- **懒加载纹理**：各 `RhiTexture*` 仅在指针为空时通过 `RenderTextureData` 创建；贴图替换时尚未完整失效重建（见下方问题 #2）。
