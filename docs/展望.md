...
`RenderMeshResource::reset()` 尚未真正释放 GPU buffer；`RenderTextureData` 创建的 `RhiTexture` 无统一 cache 与 shutdown 释放策略。

结果：同一贴图路径或同一 submesh 可能重复上传；长期存在泄漏风险。

### 2. 纹理热更新不完整

`RenderMaterialResource::updateFrom()` 仅在 `RhiTexture*` 为空时上传（代码中有 `TODO reload texture handles when the source material changes texture assets`）。贴图路径变更后，GPU 侧可能仍持有旧纹理。
...
- ImGui 预览通过 `RenderTextureData::id` 或专门 adapter 取得 native id，不扩散到 RenderPass 内部。
